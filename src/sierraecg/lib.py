    ...
    for item in get_nodes(elt_repbeats, "repbeat"):
        ...
        if waveform is not None:
            ...
            if encoding == "Base64":
                ...
                raise UnsupportedXmlFileError(f"Representative beat waveform data encoding unsupported: {encoding}")
...
def get_waveform_data(
    ...
    if compression_method != "Uncompressed":
        if compression_method == "XLI":
            ...
            raise UnsupportedXmlFileError(f"Waveform data compression algorithm unsupported: {compression_method}")
...
def split_leads(waveform_data: bytes, lead_count: int, samples: int) -> List[npt.NDArray[np.int16]]:
